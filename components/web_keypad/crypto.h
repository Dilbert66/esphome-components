...
class SHA256HMAC
{
    public:
        /**
         * Compute a SHA256 HMAC with the given [key] key of [length] chars 
         * for authenticity
         */
        SHA256HMAC(const char *key, unsigned int keyLen);
        /**
         * Update the hash with new data
         */
        ...
        bool matches(const char *expected);
    private:
        void blockXor(const char *in, char *out, char val, char len);
        SHA256 _hash;
        char _innerKey[SHA256HMAC_BLOCKSIZE];
        char _outerKey[SHA256HMAC_BLOCKSIZE];
};
