...
#define FC(s) ((const char*)(s))
        ...
        const char *const HEADER_PNA_NAME = FC("Private-Network-Access-Name");
        const char *const HEADER_PNA_ID = FC("Private-Network-Access-ID");
        const char *const HEADER_CORS_REQ_PNA = FC("Access-Control-Request-Private-Network");
        const char *const HEADER_CORS_ALLOW_PNA = FC("Access-Control-Allow-Private-Network");
#endif
...
void WebServer::handle_index_request(struct mg_connection *c)
{
    ...
    #ifdef USE_ESP8266
    uint8_t buf1[ESPHOME_WEBKEYPAD_INDEX_HTML_SIZE];
    memcpy_P(buf1, buf, ESPHOME_WEBKEYPAD_INDEX_HTML_SIZE);
    mg_send(c, buf1, ESPHOME_WEBKEYPAD_INDEX_HTML_SIZE);
    #else
...
void WebServer::handle_css_request(struct mg_connection *c)
{
    ...
    #ifdef USE_ESP8266
    uint8_t buf1[ESPHOME_WEBKEYPAD_CSS_INCLUDE_SIZE];
    memcpy_P(buf1, buf, ESPHOME_WEBKEYPAD_CSS_INCLUDE_SIZE);
    mg_send(c, buf1, ESPHOME_WEBKEYPAD_CSS_INCLUDE_SIZE);
    #else
      ...
      void WebServer::ev_handler(struct mg_connection *c, int ev, void *ev_data)
        {
            ...
            //WebServer *srv = static_cast<WebServer *>(webServerPtr);
            #ifdef USE_WEBKEYPAD_OTA
            if (c->is_ota) handle_uploads(c, ev, ev_data);
            #endif
            bool final = false;
            if (ev == MG_EV_POLL) {
