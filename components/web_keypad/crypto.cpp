...
static const char sha256_padding[64] =
{
...
void SHA256::SHA256_Process(const char digest[64])
{
 ...
 */
void SHA256::doUpdate(const char * msg, int len)
{
    ...
    if (left && len >= fill)
    {
        ...
        SHA256::SHA256_Process((const char*) buffer);
        len -= fill;
        msg  += fill;
        left = 0;
    }
 ...
 */
void SHA256::doFinal(char *digest)
{
    uint32_t last, padn;
    uint32_t high, low;
    char msglen[8];
...
SHA256HMAC::SHA256HMAC(const char *key, unsigned int keyLen)
{
    // sort out the key
    char theKey[SHA256HMAC_BLOCKSIZE];
...
void SHA256HMAC::doUpdate(const char *msg, unsigned int len)
{
    _hash.doUpdate(msg, len);
}

void SHA256HMAC::doFinal(char *digest)
{
    // compute the intermediate hash
    char interHash[SHA256_SIZE];
...
bool SHA256HMAC::matches(const char *expected)
{
    char theDigest[SHA256_SIZE];
    doFinal(theDigest);
    for (unsigned char i = 0; i < SHA256_SIZE; i++)
