    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_USESSL, default=False): cv.boolean,
            cv.Optional(CONF_USEMG, default=False): cv.boolean,            
            ...
            cv.SplitDefault(CONF_SKIP_CERT_CN_CHECK): cv.All(
                cv.boolean
            ),
...
async def to_code(config):
    ...
    if CONF_USEMG in config and config[CONF_USEMG]:
